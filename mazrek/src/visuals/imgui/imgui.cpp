...
bool ImGui::BeginButtonDropDown(const char* label, ImVec2 buttonSize)
{
	...
	ImVec2 size(24, buttonSize.y);
	bool pressed = ImGui::Button("##", size);

	// Arrow
	ImVec2 center(window->Pos.x + x + 12, window->Pos.y + y + buttonSize.y / 2);
	float r = 4.f;
	center.y -= r * .25f;
	...
	popupPos.x = window->Pos.x + x - buttonSize.x - 80;
	popupPos.y = window->Pos.y + y + buttonSize.y - 80;
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	/* Uncomment if need text near SliderFloat !*/
	/* Uncomment if need text near SliderFloat !*/
	/* Uncomment if need text near SliderFloat !*/
	/* Uncomment if need text near SliderFloat !*/
	/* Uncomment if need text near SliderFloat !*/
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y, label_size.y + style.FramePadding.y)); // We want a square shape to we use Y twice
    ItemSize(check_bb, style.FramePadding.y);

    ImRect total_bb = check_bb;
    if (label_size.x > 0)
        SameLine(0, style.ItemInnerSpacing.x);
    const ImRect text_bb(window->DC.CursorPos + ImVec2(0,style.FramePadding.y-3), window->DC.CursorPos + ImVec2(0,style.FramePadding.y-3) + label_size);
    if (label_size.x > 0)
    {
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    center.x = (float)(int)center.x + 0.4f;				// was 0.5f
    center.y = (float)(int)center.y + 0.4f;				// was 0.5f
    const float radius = check_bb.GetHeight() * 0.4f;	// was 0.5f
...
bool ImGui::ComboVec(const char* label, int* current_item, std::vector<std::pair<std::string, DWORD>>& items, int items_count, int height_in_items)
{
    return ComboWithInputSupport(label, label, current_item,
        [](void* vec, int idx, const char** out_text)
        {
            std::vector<std::pair<std::string, DWORD>>* vector = reinterpret_cast<std::vector<std::pair<std::string, DWORD>>*>(vec);
...
bool ImGui::ComboWithInputSupport(const char* label, const char* preview_text, int* current_item, bool(*items_getter)(void*, int, const char**),
...
bool ImGui::ListBoxVec(const char* label, int* current_item, std::vector<std::pair<std::string, std::string>>& items, int items_count, int height_items)
{
	const bool value_changed = ListBox(label, current_item, 
		[](void* vec, int idx, const char** out_text)
	{
		auto vector = reinterpret_cast<std::vector<std::pair<std::string, std::string>>*>(vec);
		if (idx < 0 || idx >= vector->size())return false;
